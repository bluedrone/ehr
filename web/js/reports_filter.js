...
modulejs.define('standalone/reports/filter', ['jquery', 'underscore'],
  function ($, _) {
    var filterBy = ["dateFrom", "dateTo", "clinicianName", "patientName",
      "activityName"
    ];
    ...
    return {
      process: function (data, conditions) {
        var result = $.grep(data, function (elem, ndx) {
          var matches = [true];
          var filter, conditionsVal;
          for (var ndx in filterBy) {
            filter = filterBy[ndx];
            if (conditions.hasOwnProperty(filter) && conditions[filter]) {
              if (filter == "dateFrom") {
                matches.push(moment(elementVal(elem, filter)) >= moment(
                  conditions[filter]));
              } else if (filter == "dateTo") {
                matches.push(moment(elementVal(elem, filter)) <= moment(
                  conditions[filter]));
              } else if (filter == "clinicianName" || filter ==
                "patientName" || (conditions["activityId"] != 0 &&
                  filter == "activityName")) {
