...
modulejs.define('reports', ["app", "render_util", "util"], function (App,
  RenderUtil, Util) {
  App.currentReportId;
  App.clinicianActivity;
  App.activityLogs;
  App.groupByPatientsLog;
  $('.app-reports-link').click(function () {
    viewReports();
  });

  function viewReports() {
    App.viewReportsScreen();
  ...
  function getActivityLog() {
    ...
    $.post("reports/getActivityLog", {
      data: jsonData
    }, function (data) {
      App.activityLogs = $.parseJSON(data);
      RenderUtil.render('component/simple_data_table', {
        items: App.activityLogs,
        title: 'Activity Logs',
  ...
  function getGroupByPatientsLog() {
    ...
    $.post("reports/getGroupByPatientsLog", {
      data: jsonData
    }, function (data) {
      App.groupByPatientsLog = $.parseJSON(data);
      RenderUtil.render('component/reports_nested_table', {
        items: App.groupByPatientsLog,
        title: 'Grouped by Patients Activity Logs',
        tableName: 'reports-content',
        clickable: false
      }, function (s) {
        $('#reports-content').html(s);
        $('#reports-view-header').html(
          "Grouped by Patients Activity Logs");
        $('#report-date-from').mask("99/99/9999");
        $('#report-date-to').mask("99/99/9999");
      });
    });
  }
  $('#export-csv-activity-log-lg, #export-csv-activity-log-sm').click(
    function () {
      //exportCsv();
      exportTableToCSV.apply(this, [$('#reports-content>table'),
        'ActivityLog.csv'
      ]);
    });
  ...
  function viewReport() {
    ...
    if (App.currentReportId == 25) {
      $('#report-filter').css({
        display: "block"
      });
      getActivityLog();
    } else if (App.currentReportId == 26) {
      $('#report-filter').css({
        display: "block"
      });
      getGroupByPatientsLog();
    } else {
      $('#reports-view-header').html(App.currentReportId);
    }
  }
  ...
  function initActivityLogSearchTypeAheads() {
    ...
    $.post("reports/getActivityLogSearchTypeAheads", {
      data: jsonData
    }, function (data) {
      ...
      App.clinicianActivity = parsedData.clinicianActivityList.clinicianActivity;
      $('#reports-clinician-search-full-name').typeahead({
        hint: true,
        highlight: true,
        minLength: 1
      }, {
        name: 'clinicianFullNames',
        displayKey: 'value',
        source: Util.substringMatcher(
          activityLogClinicianSearchTypeAheads.clinicianFullNames
        )
      });
      $('#reports-patient-search-full-name').typeahead({
        ...
        name: 'patientFullNames',
        displayKey: 'value',
        source: Util.substringMatcher(
          activityLogPatientSearchTypeAheads.patientFullNames)
      });
  ...
  function renderClinicianActivity() {
    RenderUtil.render('component/basic_select_options', {
      options: App.clinicianActivity,
      collection: 'app_clinicianActivity'
    }, function (s) {
  ...
  function filterActivityLog() {
    ...
    Util.debug("ActivityLog filter json data: " + jsonData);
    $.post("reports/filterActivityLog", {
      data: jsonData
    }, function (data) {
  ...
  function filterGroupByPatientsActivityLog() {
    ...
    Util.debug("Grouped by Patients Activity Logs json data: " + jsonData);
    $.post("reports/filterGroupByPatientsActivityLog", {
      data: jsonData
    }, function (data) {
      var parsedData = $.parseJSON(data);
      RenderUtil.render('component/reports_nested_table', {
  ...
  $('#report-view-button').click(function () {
    viewReport();
  });
  ...
  $('#btn-reports-activity-log-clear').click(function () {
    clearActivityLogFilter();
  });
  $('#print-report-button').click(function () {
    if (App.currentReportId == 26) {
      window.print();
      /* RenderUtil.render('print/grouped_by_patients_activity_logs',  {groupedByPatientsActivityLogs:App.groupByPatientsLog}, function(obj) {
