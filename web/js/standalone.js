			...
			if (mode.isStandalone()) {
				...
				fakeUrl = function(url) {
					return url.match(appUrlRegex) || 
						  url.match(reportsUrlRegex) ||
						  url.match(patientUrlRegex) || 
						  url.match(htmlUrlRegex) 
				}
				expectsJson = function(url) {
					switch(url) {
							...
							if (url.match(htmlUrlRegex)) {
								...
								processData = function(data) {
									if ( url.match('app/getAppointmentsByClinician')) {
										//non existing value will cause array to be returned
										postData = {id:-1}
